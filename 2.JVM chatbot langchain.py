...
faiss_db = FAISS.load_local("guidance_to_students_faiss_V3", embeddings, allow_dangerous_deserialization=True)
...
question_chain = RunnableBranch(
    (RunnableLambda(lambda x: is_english(x["question"])), direct_chain ),
    mr_to_en_chain
)
